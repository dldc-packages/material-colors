...
import { MaterialColors } from "./MaterialColors.ts";
...
export const InterpolatedMaterialColors: IInterpolatedMaterialColors = Object
  .keys(
    MaterialColors,
  ).reduce<IInterpolatedMaterialColors>((acc, key) => {
    const name: TColorName = key as any;
    const values: Points = [[0, "#ffffff"], ...MaterialColors[name], [
      1000,
      "#000000",
    ]];
